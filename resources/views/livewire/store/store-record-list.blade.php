        ...
        <div class="app-card-body">
            <div class="table-responsive">
                <table class="table app-table-hover mb-0 text-left">
                    ...
                    <tbody>
                        @forelse ($storeRecord as $record)
                            <tr>
                                <td class="cell">{{ date('F d, Y ',strtotime($record->date_of_visit)) }} </td>
                                <td class="cell">{{ $record->audit_form->user->name }}</td>
                                <td class="cell">{{ $record->received_by }}</td>
                                <td class="cell">{{ $record->wave }}</td>
                                <td class="cell">
                                    <span class="badge {{ $record->audit_form->StatusBadge }}"> {{ $record->audit_form->StatusString }} </span><br>
                                </td>


                                <td class="cell table-action-sm">
                                    <a  href="{{ route('audit.view.result', ['form_id' => $record->form_id, 'summary_id' => $record->id]) }}">
